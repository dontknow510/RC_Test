...
static uint16_t g_target_us[SERVO_CHANNEL_COUNT];
...
static uint8_t g_gyro_available;
...
{
  int32_t span = (int32_t)GIMBAL_ADC_RAW_MAX - (int32_t)GIMBAL_ADC_RAW_MIN;
  int32_t position;
  int32_t pulse_us;
  uint8_t invert;
  ...
  position = (int32_t)raw - (int32_t)GIMBAL_ADC_RAW_MIN;
  if (position < 0)
  {
    position = 0;
  }
  ...
  invert = (channel == SERVO_CHANNEL_HORIZONTAL) ? GIMBAL_HORIZONTAL_INVERT
                                                 : GIMBAL_PITCH_INVERT;
  if (invert != 0U)
  {
...
static void Gimbal_CaptureGyroBase(const MPU6050_AppState *mpu)
{
  ...
  g_gyro_base_us[SERVO_CHANNEL_HORIZONTAL] =
      g_target_us[SERVO_CHANNEL_HORIZONTAL];
  g_gyro_base_us[SERVO_CHANNEL_PITCH] = g_target_us[SERVO_CHANNEL_PITCH];
}
...
void Gimbal_Init(void)
{
  ...
  for (i = 0U; i < (uint32_t)SERVO_CHANNEL_COUNT; i++)
  {
    g_target_us[i] = SERVO_PULSE_CENTER_US;
                   ...
                   uint32_t tick)
{
  ...
  g_gyro_available = ((mpu->initialized != 0U) &&
                      (mpu->sample_valid != 0U)) ? 1U : 0U;

  /* 陀螺仪失效时自动降级回电位器模式，避免目标角失控 */
  if ((g_mode == GIMBAL_MODE_GYRO) && (g_gyro_available == 0U))
  {
    g_mode = GIMBAL_MODE_POTENTIOMETER;
    Gimbal_ResetAdcFilter();
  }
  ...
  for (i = 0U; i < (uint32_t)SERVO_CHANNEL_COUNT; i++)
  {
    g_target_us[i] = Gimbal_SlewLimit(g_target_us[i], wanted[i]);
    Servo_SetPulseUs((ServoChannel)i, g_target_us[i]);
  }

  /* --- 蓝牙发送 --- */
  if ((tick % GIMBAL_BT_SEND_TICKS) == 0U)
  {
    frame.seq = g_tx_seq;
    g_tx_seq++;
    frame.mode = (uint8_t)g_mode;
    frame.ch1_us = g_target_us[SERVO_CHANNEL_HORIZONTAL];
    frame.ch2_us = g_target_us[SERVO_CHANNEL_PITCH];
...
uint16_t Gimbal_GetTargetUs(ServoChannel channel)
{
  ...
  return g_target_us[channel];
}

uint8_t Gimbal_IsGyroAvailable(void)
{
  return g_gyro_available;
}
