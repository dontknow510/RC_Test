...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      if ((can_rx_message.std_id == CAN_MASTER_TX_ID) &&
          (can_rx_message.length >= 1U) &&
          (can_rx_message.data[0] == CAN_MASTER_DATA))
      {
        const uint8_t tx_data = CAN_SLAVE_DATA;

        HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
        (void)CAN_SendMessage(CAN_SLAVE_TX_ID, &tx_data, 1U);
      }
    }
  }
  /* USER CODE END 3 */
